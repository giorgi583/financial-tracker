...
{isProfileMenuOpen && (
  ...
  <p className="p-2 hover:bg-gray-200/50 rounded-md cursor-pointer w-full flex items-center gap-2" onClick={()=> logout()}><LogOutIcon size={17} /> {t('logout')}</p>
      ...
      <Outlet context={user}/>
      <div className="rounded-full p-5 bg-[var(--mainbg)] dark:bg-[var(--accent)] fixed bottom-10 -right-10 hover:translate-x-[-60px] max-md:hidden cursor-pointer text-gray-950 dark:text-white transition-all duration-300" onClick={()=> setIsChatOpen(!isChatOpen)}><MessageSquareMoreIcon
