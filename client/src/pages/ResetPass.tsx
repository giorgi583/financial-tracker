  ...
  return (
    <div className="flex items-center justify-center min-h-screen gap-4 bg-gray-50">
      ...
      <form onSubmit={handlePasswordChange} className="mt-4 flex flex-col gap-4">
       <div className='relative w-full'><Lock className='size-4 text-gray-500 absolute top-3 left-3' /><input name="password" id="password" value={password} onChange={(e)=> setPassword(e.target.value)} type= {showPassword ? "text" : "password"} placeholder="new password" className="border border-gray-300 rounded-lg p-2 focus:outline-none focus:ring-2 focus:ring-blue-500 w-full pl-9" />{!showPassword ? <EyeIcon className='cursor-pointer size-5 text-gray-500 absolute top-3 right-3' onClick={() => setShowPassword(!showPassword)} /> : <EyeOff className='cursor-pointer size-5 text-gray-500 absolute top-3 right-3' onClick={() => setShowPassword(!showPassword)} />}</div> 
        <div className='relative w-full'><Lock className='size-4 text-gray-500 absolute top-3 left-3' /><input name="password" id="password" value={password} onChange={(e)=> setPassword(e.target.value)} type={showPassword ? "text" : "password"} placeholder="confirm password" className="border border-gray-300 rounded-lg p-2 focus:outline-none focus:ring-2 focus:ring-blue-500 w-full pl-9" />{!showPassword ? <EyeIcon className='cursor-pointer size-5 text-gray-500 absolute top-3 right-3' onClick={() => setShowPassword(!showPassword)} /> : <EyeOff className='cursor-pointer size-5 text-gray-500 absolute top-3 right-3' onClick={() => setShowPassword(!showPassword)} />}</div> 
