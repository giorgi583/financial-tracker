  ...
  return (
    <div className="p-8 w-full max-md:p-5 max-sm:p-2 max-sm:mt-10 max-sm:pb-25">
      ...
      <div className="flex gap-2 items-center mt-4 border border-gray-300 rounded-lg p-1 max-w-fit shadow-sm">
        <button onClick={() => setSetcion('overview')} className={`${setcion === 'overview' ? 'btn rounded-md py-1 px-3 text-slate-700 cursor-pointer font-semibold' : 'btn rounded-md py-1 px-3 text-slate-700 cursor-pointer '}`}>{t('overview')}</button>
        <button onClick={() => setSetcion('limits')} className={`${setcion === 'limits' ? ' btn rounded-md py-1 px-3 text-slate-700 cursor-pointer font-semibold' : 'btn rounded-md py-1 px-3 text-slate-700 cursor-pointer '}`}>{t('limits')} & {t('goals')}</button>
        <button onClick={() => setSetcion('alerts')} className={`${setcion === 'alerts' ? ' btn rounded-md py-1 px-3 text-slate-700 cursor-pointer font-semibold' : 'btn rounded-md py-1 px-3 text-slate-700 cursor-pointer'}`}>{t('alerts')}</button>
      </div>
