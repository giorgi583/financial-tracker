...
const ProfileSettings = () => {
  ...
  return (
    <div className="p-10 max-sm:pt-10 max-sm:pb-20 max-sm:m-auto">
        ...
        <button onClick={(e) => {e.preventDefault(); setChangePassword(true)}} className='px-3 py-1 bg-linear-to-tl from-blue-200 to-blue-100 text-blue-700 rounded-md cursor-pointer self-start'>{t('updatePassword')}</button>
        ...
        <div className='flex flex-col gap-10 max-md:order-1 max-md:flex-row max-md:items-start max-md:w-full'>
          ...
          <button onClick={(e) => { e.preventDefault(); setShowUploadButton(!showUploadButton)}} className='absolute bottom-0 right-0 cursor-pointer text-lg text-gray-950 flex bg-slate-100  items-center gap-1 border border-gray-300 rounded-lg py-1 px-3'>{t('edit')}<NotebookPenIcon color='black'/></button>
          <label htmlFor="fileInput" className={`text-lg absolute -bottom-10 px-3 py-1 right-0 text-gray-950 border-2 border-gray-300 rounded-md bg-slate-200 cursor-pointer ${showUploadButton ? 'block' : 'hidden'} hover:scale-105 transition-transform duration-500`}>Upload</label>
          <input type="file" accept="image/*" className='hidden' id="fileInput" onChange={handleFileChange} />
        </div>
        <button className='px-3 py-1 bg-linear-to-tl from-blue-200 to-blue-100 text-blue-700 rounded-md cursor-pointer'>{t('saveChanges')}</button>
        </div>
        </form>
        </div>
  )
