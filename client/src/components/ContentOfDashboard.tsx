...
useEffect(() => {
  ...
  return (
    <div className={className}>
      ...
      {title === 'Top categories' && <div className="mt-1 border border-[var(--sidebar)]"> <div className="grid grid-cols-3 text-lg max-2xl:text-sm font-semibold border border-[var(--accent)]"><p className="text-left px-2 py-2 max-2xl:px-1">Category</p> <p className="text-right px-2 py-2 max-2xl:px-1">Amount</p> <p className="text-right px-2 py-2 max-2xl:px-1">Percentage</p></div> {data?.data?.topCategories.map ((transaction: any, i: number) => <div key={transaction.id} className={`grid grid-cols-3  w-full`}><p className="text-left border border-r-0 border-[var(--accent)] px-2 py-2 max-2xl:px-1">{transaction.category}</p> <p className="text-right border-y border-[var(--accent)] px-2 py-2 max-2xl:px-1">{transaction.amount} {currencySign}</p> <p className="text-right border border-l-0 border-[var(--accent)] px-2 py-2 max-2xl:px-1">{transaction.percentage}%</p> </div>)} </div>}
      </div> 
      </>
       : 
